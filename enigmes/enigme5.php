...
$enigma = $gameFunctions->getCurrentEnigma(5);

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['answer'])) {
    $answer = clean_input($_POST['answer']);
    $correct = $gameFunctions->checkAnswer($_SESSION['user_id'], 5, $answer);
...
<head>
    ...
    <title>Énigme 5 - Déchiffrement</title>
...
<body>
    ...
    <main class="container">
        <div class="game-container">
            <h2>Niveau 5: Cybersécurité - Déchiffrement</h2>
            ...
            <div class="congratulations" id="congratsMessage">
                <h3>Félicitations !</h3>
                <p>Vous avez sauvé le laboratoire quantique!</p>
            </div>
            
            <div class="enigma-card">
                <div class="enigma-description">
                    <p>Dernière étape ! Les pirates ont haché le mot de passe final. Déchiffrez-le pour reprendre le contrôle total du laboratoire.</p>
                    <p><strong>Énoncé:</strong> <?= nl2br(htmlspecialchars($enigma['consigne'])) ?></p>
                    <p><strong>Mot de passe haché (MD5):</strong> <code>5f4dcc3b5aa765d61d8327deb882cf99</code></p>
                    <p><strong>Indice:</strong> C'est l'un des mots de passe les plus courants au monde</p>
                </div>
                
                <form method="POST" action="enigme5.php?mode=<?= htmlspecialchars($mode) ?>" id="finalForm">
                    <div class="form-group">
                        <label for="answer">Mot de passe original:</label>
                        <input type="text" id="answer" name="answer" required class="form-control">
                        <small class="form-text text-muted">Le hash MD5 correspond à un mot de passe très simple de 8 caractères</small>
                    </div>
                    <button type="submit" class="btn btn-success">Valider la solution finale</button>
                </form>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        // Special handling for final enigma
        document.getElementById('finalForm').addEventListener('submit', function(e) {
            const answer = document.getElementById('answer').value.toLowerCase();
            if (answer === 'password') {  // MD5 of 'password' is 5f4dcc3b5aa765d61d8327deb882cf99
